...
export async function getData(locationName, lazyLoadIteration = 0) {
  ...
  const postsResult = await executeQuery({
    query: postsQuery,
    values: [locationName, lazyLoadIteration * 10],
  });
  ...
  return [false, { postsData, ...countResult[0] }];
}
...
export default async function handler(req, res) {
  ...
  const { locationName, lazyLoadIteration } = req.query;
  const [error, data] = await getData(locationName, lazyLoadIteration);
